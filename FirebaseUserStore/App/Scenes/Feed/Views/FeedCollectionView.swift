    ...
    private var verticalGrid: some View {
        NavigationView {
                ScrollView(showsIndicators: false) {
                    ...
                    } else if store.state.items.count > 0 {
                        ItemsSectionView(items: store.state.items, title: "Title", subtitle: "Subtitle")
...
//                    }
    ...
    private func asyncUpdate() async -> Void {
        searchBinding.isEmpty ? store.dispatch(.updateFeed) : store.dispatch(.search(text: searchBinding))
